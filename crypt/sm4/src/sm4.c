 ...
 * @return 1 Failed
 */
static int sm4_make_enc_subkeys(const uint8_t key[SM4_KEY_SIZE], uint32_t encSubKeys[SM4_ROUNDS])
{
    uint32_t MK[4];
    uint32_t k[36];
    uint32_t i = 0;
 ...
 * @return 1 Failed
 */
static int sm4_make_dec_subkeys(const uint8_t key[SM4_KEY_SIZE], uint32_t decSubKeys[SM4_ROUNDS])
{
 ...
 */
static void sm4_encrypt_block(const uint8_t *input, const uint32_t encSubKeys[SM4_ROUNDS], uint8_t *output)
{
    uint32_t buff[36];
    uint32_t tmp;
    for (int i = 0; i < 4; i++)
 ...
 */
static void sm4_decrypt_block(const uint8_t *input, const uint32_t decSubKeys[SM4_ROUNDS], uint8_t *output)
{
    uint32_t buff[36];
    uint32_t tmp;
    for (int i = 0; i < 4; i++)
